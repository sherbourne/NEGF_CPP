...
int main(int argc, char **argv) {
	...
	double 			ElectronDensitySpectral_E[Nx],     	ElectronDensitySpectral[NE][Nx], 
					CurrentDensitySpectral_E[Nx-1],  	CurrentDensitySpectral[NE][Nx-1],
	...
	if (pid > 0){
		...
		for (int j=0; j<Nx; j++){
			...
			LDOS_E[j] 					 = G_RT(j,j).imag()*2/(pi*dx);
			  if (j!=Nx-1){
			CurrentDensitySpectral_E[j] = (2*q*t/h)*(G_lesT(j+1,j).real() - G_lesT(j,j+1).real());
			   }
		}
			...
			MPI_Send(&CurrentDensitySpectral_E, Nx-1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
	}
		...
		if (pid == 0) {
			...
			for (unsigned np=0; np<NE; np++){
				...
				MPI_Recv(&CurrentDensitySpectral[np], Nx-1, MPI_DOUBLE, np+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			}			  
			...
			if (PoissonEps<PoissonConvergence){
				...
				ofstream Potential_out, EDT_out, EDS_out, LDOS_out, CDS_out;
				...
				CDS_out.open 		("CurrentDensitySpectral.dat");
				...
				endtime = MPI_Wtime();
					...
					for (int i=0; i<NE; i++){
						for (int j=0; j<Nx ;j++){
								...
								LDOS_out 	<<	LDOS[i][j]						 <<	" ";
								if (j!=Nx-1){
								CDS_out 	<< 	CurrentDensitySpectral[i][j]	 << " ";	
								}
						}
						        EDS_out 	<<	endl;
								LDOS_out 	<<	endl;
